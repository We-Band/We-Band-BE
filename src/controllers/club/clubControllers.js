...
//동아리 정보 조회 API (GET /clubs/:clubId)
export const viewClub = async (req, res) => {
  try {
    const { clubId } = req.params;
    const club = req.club;

    const { club_name, club_code, club_img, member_count, club_leader } = club;
...
export const kickMember = async (req, res) => {
  try {
    ...
    //추방할 user랑 회장 아이디랑 같으면 추방 불가능(회장은 본인이 추방할 수 없음)
    if (userId == myId) {
    ...
    return res
      .status(500)
      .json({ message: "동아리 추방 중 오류가 발생했습니다." });
  }
};
...
export const changeCode = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "동아리 가입 코드가 수정 되었습니다." });
  } catch (error) {
    logger.error(`동아리 코드 변경 중 오류 발생: ${error.message}`);
    return res
      .status(500)
      .json({ message: "동아리 코드 변경 중 오류가 발생했습니다." });
  }
};
...
export const changeLeader = async (req, res) => {
  try {
    ...
    if (!newLeader) {
      ...
      return res
        .status(400)
        .json({ messsage: "회장으로 임명할 사용자 정보가 누락 되었습니다." });
    }
    ...
    logger.info(
      `동아리 ${clubId}에 회장이 사용자: ${newLeader}로 변경되었습니다.`
    );
    ...
    return res
      .status(500)
      .json({ message: "동아리 회장 변경 중 오류가 발생했습니다." });
  }
};
