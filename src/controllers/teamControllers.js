...
export const getTeam = async (req, res) => {
  try {
    ...
    const teams = await teamService.getTeam({ clubId, type, userId });
...
export const viewTeam = async (req, res) => {
  try {
    ...
    const teamInfo = await teamService.viewTeam({ teamId });

    logger.debug("팀 조회 성공", { teamId, teamInfo });
...
export const changeTeamProfile = async (req, res) => {
  try {
    ...
    await teamService.changeTeamProfile({
      teamId,
      file: req.file,
    });
...
export const changeTeamName = async (req, res) => {
  try {
    ...
    logger.debug("팀 이름 변경 성공", { teamId, teamName });
    return res
      .status(200)
...
export const changeTeamLeader = async (req, res) => {
  try {
    ...
    logger.debug("팀장 변경 성공", { teamId, newLeader });
    return res
      .status(200)
...
export const addTeamMembers = async (req, res) => {
  try {
    ...
    logger.debug("팀원 추가 성공", { teamId, members });
    return res
      .status(200)
...
export const kickTeamMember = async (req, res) => {
  try {
    ...
    logger.debug("팀원 삭제 성공", { teamId, userId });
    return res
      .status(200)
...
export const leaveTeam = async (req, res) => {
  try {
    ...
    logger.debug("팀 탈퇴 성공", { teamId, userId });
    return res
      .status(200)
      .json({ message: "팀에서 성공적으로 탈퇴되었습니다." });
  } catch (error) {
    logger.error(`팀 탈퇴 검증 실패: ${error.message}`, { error });
