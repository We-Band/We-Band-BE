...
export const verifyClubSchedule = async (req, res, next) => {
  try {
    ...
    logger.error(`동아리 일정 존재 여부 검증 실패: ${error.message}`, {
      error,
    });
    return res
      .status(500)
      .json({ message: "동아리 일정 존재 여부 검증 중 오류 발생" });
  }
};

// 동아리 일정 정보 누락 검증 미들웨어
export const isMissingClubSchedule = async (req, res, next) => {
  try {
    ...
    if (!clubScheduleStart && !clubScheduleEnd) {
      ...
      return res
        .status(400)
        .json({ message: "동아리 일정 시간이 누락되었습니다." });
    }
    ...
    if (!clubScheduleTitle) {
      ...
      return res
        .status(400)
        .json({ message: "동아리 일정 제목이 누락되었습니다." });
    }
    next();
  } catch (error) {
    ...
    return res
      .status(500)
      .json({ message: "동아리 일정 누락 검증 중 오류 발생" });
  }
};
