...
export const verifyTeamSchedule = async (req, res, next) => {
    try {
        ...
        const teamSchedule = await prisma.teamSchedule.findUnique({
            where: {
        ...
        return res.status(500).json({ message: "팀 일정 존재 여부 검증 중 오류 발생" });
    }
};
...
export const isMissingTeamSchedule = async (req, res, next) => {
        try {  
            ...
            if (!teamScheduleStart && !teamScheduleEnd) {
                ...
                return res.status(400).json({ message: "팀 일정 시간이 누락되었습니다." });
            }

            if (!teamScheduleTitle) {
                logger.debug("팀 일정 제목 누락");
                return res.status(400).json({ message: "팀 일정 제목이 누락되었습니다." });
            }
            next();   
        }catch(error) {
